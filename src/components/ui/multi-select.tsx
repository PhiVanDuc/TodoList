  ...
  return (
    <PopoverTrigger asChild>
      <Button
        ...
        className={cn(
          "flex h-auto w-fit items-stretch justify-between gap-2 overflow-hidden rounded-md border border-input bg-transparent p-[8px] text-sm whitespace-nowrap shadow-xs transition-[color,box-shadow] outline-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-destructive/20 data-[placeholder]:text-muted-foreground dark:bg-input/30 dark:hover:bg-input/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 [&_svg:not([class*='text-'])]:text-muted-foreground",
          className,
        )}
      >
        {children}

        <span className="self-stretch flex items-center"><ChevronsUpDownIcon className="size-4 shrink-0 opacity-50" /></span>
      </Button>
    </PopoverTrigger>
  )
}
...
export function MultiSelectValue({
  ...
  if (selectedValues.size === 0 && placeholder) {
    return (
      <div className="h-[28px] flex items-center px-[4px]">
      ...
      {[...selectedValues]
        ...
        .map(value => (
          <Badge
            variant="outline"
            data-selected-item
            className="group h-[28px] flex items-center gap-1"
            key={value}
            onClick={
              clickToRemove
                ? e => {
